...
-- ============================================================
-- 部署前請注意：
--  - config.js 的 SUPABASE_URL / SUPABASE_ANON_KEY 要換成您自己專案的值
--  - facility_lookup 的 CSV 上傳格式為 4 欄：管制編號,名稱,住址,分類方式
--    （分類方式決定 admin.html 日曆檢視的顏色分組，可以是縣市，也可以是任何自訂分類）
-- ============================================================
...
create table campaigns (
  ...
  -- 格式: {"管制編號": {"name":"工廠名稱","address":"完整住址","category":"分類方式"}, ...}
  -- 業者填表輸入管制編號時用來自動帶出工廠名稱與住址；category 用於 admin.html 日曆檢視的顏色分組
  -- （為相容舊資料，前端讀取時也接受陣列格式 ["工廠名稱","縣市"]）
...
create table submissions (
  ...
  constraint submissions_campaign_control_no_unique unique (campaign_id, control_no)
  -- 同一案件內管制編號不可重複：業者重新送出表單時會覆蓋更新同一筆，而不是新增重複資料
);
...
-- 業者送出表單用的 RPC：同一管制編號重複送出會「覆蓋更新」同一筆資料（而不是新增），
-- 讓業者填錯資料時可以直接重填一次修正。內建 30 秒頻率限制（看 updated_at），
-- 防止公開表單被自動化程式或誤觸連續送出洗版
...
begin
  ...
  on conflict (campaign_id, control_no) do update set
